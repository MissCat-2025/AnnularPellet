...
registerMooseObject("AnnularPelletApp", GapFluxModelRossStouteContactConduction);
...
GapFluxModelRossStouteContactConduction::computeFlux() const
{
  ...
  if (_qp >= _contact_pressure.size() ||
      _qp >= _primary_conductivity.size() ||
      _qp >= _secondary_conductivity.size() ||
      _qp >= _primary_hardness.size() ||
      _qp >= _secondary_hardness.size() ||
      _qp >= _primary_T.size() ||
      _qp >= _secondary_T.size())
    return 0.0;
